...
class MyWritingViewController: UIViewController {
    ...
    private func setData() {
        for index in 0...3 {
            let dataModel = TableViewModel(index: index, date: "8월 27일", location: "용산 아이팤몰", rules: "11vs11/남성매치/풋살화 필수", fee: 7000, temaInfo: "모두의축구")
            self.tableViewModel.append(dataModel)
        }
    }
}

extension MyWritingViewController: UITableViewDelegate {
...
extension MyWritingViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "MyWritingTableViewCell", for: indexPath) as? MyWritingTableViewCell else { return UITableViewCell() }
...
extension MyWritingViewController: MyWritingTableViewCellDelegate {
    func deleteButtonDidSelected(tableViewModel: TableViewModel) {
